...
const RATING_CONFIG = {
  ...
  good: { color: 'text-blue-400', bg: 'bg-blue-500/15', label: 'Good' },
  average: { color: 'text-yellow-400', bg: 'bg-yellow-500/15', label: 'Average' },
  'needs-improvement': { color: 'text-orange-400', bg: 'bg-orange-500/15', label: 'Needs Improvement' },
...
const ResultsPanel = ({ questions, feedbacks, role, onRestart }) => {
  ...
  const getOverallRating = (avg) => {
    ...
    if (avg >= 7) return { label: 'Good Performance', color: 'text-blue-400', message: 'Solid answers overall. Focus on the improvement areas below to level up.' };
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="glass-panel rounded-2xl p-6">
        ...
        <div className="space-y-3">
          {questions.map((q, i) => {
            ...
            return (
              <details key={q.id} className="group bg-surface-container-low rounded-xl border border-white/5 overflow-hidden">
      ...
      <button
        onClick={onRestart}
        className="w-full bg-surface-container-high hover:bg-white/10 text-white font-medium py-3 rounded-xl transition-all flex items-center justify-center gap-2 border border-white/5"
      >
        <RotateCcw size={18} />
        Start New Interview
      </button>
...
const MockInterview = () => {
  ...
  return (
    <div className="max-w-6xl mx-auto h-full flex flex-col">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-white mb-1">Mock Interview</h1>
